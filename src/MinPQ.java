...
public class MinPQ<E extends Comparable<E>> implements PriorityQueueADT<E> {
	...
	private void heapify(int mode) {
		...
		if (mode == 0) {
			...
			// Loop until correct spot is found or node reaches top of the heap
			// array
			while (!heapified) {
				...
				// If child is larger than parent, then swap positions of the
				// two
		...
		if (mode == 1) {
			...
			while (!heapified) {
				...
				// Picks the smaller element to be the minChild
